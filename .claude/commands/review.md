---
description: Review a pull request (or the working diff) against this package's conventions, reporting only findings worth acting on.
argument-hint: "[PR number] [--post] [--since <sha>]"
---

Review code changes to socialmixr and report what is worth acting on.

This command runs on **your** account, with whatever model you are using. It is
committed to the repo so that anyone contributing here — maintainer or not — can
get the same review before a human looks at the PR. Nothing about it is specific
to one person's setup, and it costs the repo nothing.

Using a tool other than Claude Code? This file is the whole specification. Point
your assistant at it and ask it to follow the instructions below.

## What to review

Read `$ARGUMENTS` for a PR number and flags. Then pick the diff:
