...
The post-processing functions (`symmetrise()`, `split_matrix()`, `per_capita()`) take this as a `data.frame` with columns `lower.age.limit` (the lower end of each age group) and `population`, at the matrix's age groups. If your population data are at a different age resolution, coarsen them to the matrix's age limits with `pop_age()` first. Matrices with additional groupings take one extra column per grouping (e.g. `part_gender`).
...
For recent UN World Population Prospects data, the `wpp2024` package is available from GitHub (`remotes::install_github("PPgp/wpp2024")`). The 1-year bands are coarsened to the matrix's age limits with `pop_age()`:
