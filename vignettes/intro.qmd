...
For recent UN World Population Prospects data, the `wpp2024` package (GitHub: `PPgp/wpp2024`) provides `popAge1dt`, a table of one-year population bands. Load it from that package, relabel the ages with `limits_to_age_groups()`, and coarsen to the matrix's age groups with `align_ages()`. To keep this section self-contained we use a small stand-in with the same columns:

```{r}
# stand-in for wpp2024's popAge1dt (one-year bands; population in thousands)
