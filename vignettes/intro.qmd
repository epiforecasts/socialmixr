...
The recommended way to compute a contact matrix is a small, composable pipeline:
...
Each step is a separate function. Users who want a single call can use the `contact_matrix()` wrapper described at the end.
...
To get an idea of the uncertainty in the contact matrices, participants can be resampled with replacement. The pipeline can be iterated over bootstrap samples by resampling participant IDs:

```{r}
uk <- polymod[country == "United Kingdom"] |>
  assign_age_groups(age_limits = c(0, 1, 5, 15))

ids <- unique(uk$participants$part_id)
m <- replicate(
  n = 5,
  {
    sampled_ids <- sample(ids, replace = TRUE)
    compute_matrix(uk[part_id %in% sampled_ids])
  }
)
...
`contact_matrix()` also exposes a `sample_participants = TRUE` argument that encapsulates this bootstrap resampling.
...
As a convenience, `survey_country_population()` looks up country-specific population data from the `wpp2017` data shipped with socialmixr:
...
# Convenience wrapper: `contact_matrix()`

For simple cases, the wrapper `contact_matrix()` bundles the pipeline into a single call with named arguments:
...
It also exposes a `sample_participants = TRUE` argument for bootstrap resampling. For finer control (chained weights, custom post-processing, alternative population data sources), the pipeline functions are the recommended interface.
