...
For custom or more recent population data, you can pass a data frame directly via the `survey_pop` argument. The data frame should have columns `lower.age.limit` (the lower bound of each age group) and `population` (the number of people in each age group):
...
uk_pop <- wpp2024::popAge1dt[name == "United Kingdom" & year == 2020,
