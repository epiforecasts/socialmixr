...
Some diaries limit how many contacts a participant can list individually. In POLYMOD, the recorded number of contacts is right-censored at a diary limit that varies by country (around 29 in the United Kingdom, for example), and Belgium, Germany, Finland and the Netherlands asked participants to estimate their professional contacts rather than list each one [@mossong_social_2008]. socialmixr computes matrices from the contacts as recorded, so for these surveys the counts for high-contact participants are under-estimates. Correcting for this properly means modelling the censoring, which is out of scope; if you need to account for it, adjust the contact data before passing it to the pipeline.
...
Obtaining symmetric contact matrices, splitting out their components (see below) and population-based participant weights require information about the underlying demographic composition of the survey population.

The post-processing functions (`symmetrise()`, `split_matrix()`, `per_capita()`) need population data. In practice you supply a population table with an `age` column of age-group labels, a `population` column, and a column per other grouping, then align it to a computed matrix with `align_ages()`. `align_ages()` aggregates each grouping to the matrix's levels (coarsening the age grouping where needed) and returns the `data.frame` the post-processing functions expect.

For recent UN World Population Prospects data, the [`wpp2024`](https://github.com/PPgp/wpp2024) package (`remotes::install_github("PPgp/wpp2024")`) provides `popAge1dt`, a table of one-year population bands. Load it from that package, filter to the country and year you need, relabel the ages with `limits_to_age_groups()`, and coarsen to the matrix's age groups with `align_ages()`. Here we use a small mock-up with the same columns:
...
## When your population is coarser than your age groups

socialmixr aggregates population data into coarser age groups, and errors if you ask for age groups finer than the data provides. Splitting a band means assuming how people are distributed within it, which is a demographic modelling choice. The same applies when the population stops below the top of the matrix, for instance an 80+ table with age groups reaching to 90, and to `contact_matrix(weigh_age = TRUE)`, which builds its weighting reference at single-year resolution and so needs single-year bands. The pipeline's `weigh_by_age()` weights at the population's own bands, so it takes the population as it is.

Demographers call the splitting step graduation or ungrouping, and there are packages that do it properly. [DemoTools](https://github.com/timriffe/DemoTools) offers the classical methods and some modern ones behind a single `graduate()` call; [ungroup](https://cran.r-project.org/package=ungroup) implements the penalized composite link model. Split the population there, then pass the result to socialmixr:
...
# split into single years with DemoTools (Sprague multipliers).
# DemoTools is not a dependency of socialmixr, so it is not installed when the
# package is checked; the linter is told not to look for it.
# nolint next: namespace_linter.
single_year <- DemoTools::graduate(
  Value = five_year$population,
  Age = age_groups_to_limits(five_year$age),
  method = "sprague"
)

single_limits <- as.integer(names(single_year))

# for the pipeline functions, which take age-group labels
uk_pop <- data.frame(
  age = limits_to_age_groups(single_limits, notation = "brackets"),
  population = as.numeric(single_year)
)

# for contact_matrix(), whose survey_pop takes lower age limits
uk_survey_pop <- data.frame(
  lower.age.limit = single_limits,
  population = as.numeric(single_year)
)
```

The two shapes differ because `align_ages()` and the post-processing functions work with age-group labels, while `contact_matrix()` takes a `lower.age.limit` column.

The methods differ in how much smoothness they impose and what they assume about neighbouring bands, so which one suits your data is a question for those packages' documentation. Both provide methods rather than data, so you still need population counts from elsewhere.

For the rest of this vignette we use a small constructed `uk_pop` for the United Kingdom, sufficient to illustrate the post-processing functions:
...
To get this version of the contact matrix, compute the matrix and pass the population through `align_ages()` to `symmetrise()`:
...
`weigh()` is composable: each call multiplies new weights into the participants' `weight` column. Common recipes are wrapped as `weigh_by_*()` convenience functions:
...
`weigh_by_dayofweek()` assigns weekday participants a total weight of 5 and weekend participants a total weight of 2 (the weekly 5/2 split). `weigh_by_age()` post-stratifies against a target population: it bins participants into `uk_pop`'s age bands and multiplies in the ratio of target to observed age share within each band.

For arbitrary discrete joins, `weigh()` itself takes a two-column data frame whose key column matches `by` — e.g. pooling participants across countries by a target share:
