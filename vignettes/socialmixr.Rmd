...
To get an idea of uncertainty of the contact matrices, a bootstrap can be used using the `contact_matrix_multi()` function. If an argument `n` greater than 1 is passed to `contact_matrix_multi()`, multiple samples of contact matrices are generated. If this is combined with setting `sample.participants = TRUE` then for each sample, participants are sampled (with replacement, to get the same number of participants of the original study). All resulting contact matrices are returned as `matrices` field in the returned list. From these, derived quantities can be obtained, for example the mean:

```{r}
m <- contact_matrix_multi(n = 5, polymod, countries = "United Kingdom", age.limits = c(0, 1, 5, 15))
length(m$matrices)
