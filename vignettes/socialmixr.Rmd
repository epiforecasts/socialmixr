...
If no `age_limits` are supplied to `assign_age_groups()`, age groups are inferred from participant and contact ages.

For users who just need the matrix in a single call, the convenience wrapper `contact_matrix()` is still available; see the "Convenience wrapper" section below.
...
To get an idea of the uncertainty in the contact matrices, participants can be resampled with replacement. The easiest way to iterate the pipeline over bootstrap samples is to resample participant IDs and filter the survey before computing the matrix:

```{r}
uk <- polymod[country == "United Kingdom"]
uk <- assign_age_groups(uk, age_limits = c(0, 1, 5, 15))
...
As a convenience when processing a survey with a `country` column, `survey_country_population()` looks up country- and year-specific population data:

```{r}
survey_country_population(polymod, countries = "United Kingdom")
```

This uses the older `wpp2017` data that ships with `socialmixr` and is kept for backwards compatibility. For new work we recommend supplying more recent population data explicitly.
...
polymod[country == "United Kingdom"] |>
  assign_age_groups(age_limits = c(0, 1, 5, 15)) |>
  compute_matrix() |>
  symmetrise(survey_pop = survey_country_population(polymod, "United Kingdom"))
```

`contact_matrix()` also exposes a `sample_participants = TRUE` argument for bootstrap resampling, which may be a convenient way to obtain uncertainty intervals in a single call. For any step requiring finer control (chained weights, custom post-processing, alternative population data sources) the pipeline functions are the recommended interface.
