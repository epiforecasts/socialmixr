...
For recent UN World Population Prospects data, the `wpp2024` package is available from GitHub (`remotes::install_github("PPgp/wpp2024")`). It comes as 1-year `lower.age.limit` bands, which `regroup_ages()` coarsens to the matrix's age groups:
...
To get this version of the contact matrix, compute the matrix and pass the population through `regroup_ages()` to `symmetrise()`:
