...
The `filter` argument to `contact_matrix` can be used to select particular participants or contacts. For example, in the `polymod` dataset, the indicators `cnt_home`, `cnt_work`, `cnt_school`, `cnt_transport`, `cnt_leisure` and `cnt_otherplace` take value 0 or 1 depending on where a contact occurred:

```{r message=FALSE, warning=FALSE}
head(polymod$contacts)
```

In order to derive a contact matrix for home contacts only, one would use `filter = list(cnt_home = 1)`.

```{r message=FALSE, warning=FALSE}
contact_matrix(survey = polymod, age.limits = c(0,18,60), filter = list(cnt_home = 1))
```

In order to derive a contact matrix outside home, school or work, one would use

```{r message=FALSE, warning=FALSE}
contact_matrix(survey = polymod, age.limits = c(0,18,60), filter = list(cnt_home = 0, cnt_school = 0, cnt_work = 0))
```
...
This survey data results in an unweighted average number of contacts of 
...
The contact matrices can be plotted with the `matrix_plot()` function as a grid of colored rectangles with the numeric values in the cells. Heat colors are used by idefault, though this can be changed.
