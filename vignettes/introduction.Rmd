...
If demographic information is used, this is returned by `contact_matrix` as the `demography` field in the results list. It is possible to enfore or prevent the function to return demography data by using the boolean `return.demography`.
...
Temporal weights are activated in `contact_matirx` by `weigh.dayofweek` and age-specific weights are used when enabling `weight.age`. The post-strafitication weights are calculated by default. It is possible to obtain the participant weights via the option `return.part.weights`.
...
If the survey population differs extensively from the demography, (some) participants can end up with high weights and as such, a large contribution to the population averages. It can be warranted to limit the influence of single participants by a truncation of the weights. This trucation is applied on the standardized weights,  followed by another standardisation to make shure that the sum of the weights still equals the sample size. The latter can lead to final weights of which some little exceed the threshold value. To enbable the threshold in `contact_matrix`, you need to provide a numeric `weight.threshold`.
