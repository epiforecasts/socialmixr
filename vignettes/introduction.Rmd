...
This generates a contact matrix from the UK part of the POLOYMOD study, with age groups 0-1, 1-5, 5-15 and 15+ years. It contains the mean number of contacts that each member of an age group (row) has reported with members of the same or another age group (column).
...
To get an idea of uncertainty of the contact matrices, a bootstrap can be used. If an argument `n` greater than 1 is passed to `contact_matrix`, multiple samples of contact matrices are generated. For each sample, participants are sampled (with replacement, to get the same number of participants of the original study), and contacts are sampled from the set of all the contacts of all the participants (again, with replacement). The resulting contact matrices are returned, from which derived quantities can be obtained, for example the mean
...
By default, socialmixr uses the POLYMOD survey. A reference for any given survey can be obtained using `survey_citation`, e.g.

```r
survey_citation("polymod")
```
