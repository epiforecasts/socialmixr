...
Please note that if the option `symmetric=TRUE` is specified in `contact_matrix`, the contact matrix $m_{ij}$ can show asymmetry if the sub-population sizes are different. This is not the case for the contact matrix per capita, which will be fully symmetric:
...
The function `contact_matrix` enables the filtering of social contact data. This can for example be used to select contacts reported at school or to select long duration contacts. Any filter can applied to the data, if given as a list of the form (column=filter_value). As such, only contacts that have 'filter_value' in 'column' will be considered for the generated contact matrix.
...
The contact matrices can be plotted for example in line with the figures from Mossong et al (2020) by using the `geom_tile` function of the `ggplot2` package.
...
The contact matrices can be plotted with `matrix_plot` as a grid of colored rectangles with the numeric values in the cells. Heat colors are used by default, though this can be changend.
...
Temporal weights are activated in `contact_matirx` by `weigh.dayofweek` and age-specific weights by `weight.age`. The post-stratification weights are calculated by default. It is possible to obtain the participant weights via the option `return.part.weights`.
...
We will apply the weights by age and day of week separately in these examples, though the combination is straightforward via multiplication. With these numeric examples, we show the importance of post-stratification weights in contrast to using the crude weights directly within age-groups.

### Survey data
We start from a survey including 6 participants of 1, 2 and 3 years of age. The ages are not equally represented in the sample, though we assume they are equally present in the reference population. We will calculate the weighted average number of contacts by age and by age group, using {1,2} and {3} years of age. The following table shows the overall number of contact per age $i$ represented by $m_i$:
...
and age-specific unweighted averages on the daily number of contacts:
```{r, include=TRUE,echo=F}
...
Use the population-based weights, we are able to calculate the weighted number of contacts:
