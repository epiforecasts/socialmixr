polymod_age_processed <- process_ages(polymod)

test_that("process_ages() adds correct columns", {
  expect_true(all(
...
test_that("process_ages() appropriately changes dimensions", {
  expect_snapshot(
...
polymod_age_processed_0_5_10 <- polymod |>
  process_ages(age_limits = c(0, 5, 10))

# test the same levels
polymod_age_processed_5_10_15 <- polymod |>
  process_ages(age_limits = c(5, 10, 15))

test_that("process_ages() appropriately changes age.group factor", {
  expect_snapshot(
