...
"part_age_est_min" and "part_age_est_max". Uses mean imputation,
sampling (hot deck), or leaves them as missing. These are controlled by
...
The participant data, potentially with participant ages imputed
depending on the `estimate` method and whether age columns are present
in the data.
